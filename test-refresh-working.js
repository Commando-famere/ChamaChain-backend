...
function decryptResponse(encryptedResponse) {
    try {
        // Step 1: Base64 decode the data field
        const base64Decoded = Buffer.from(encryptedResponse.data, 'base64').toString('utf8');
        
        // Step 2: Split into IV and encrypted hex
        const parts = base64Decoded.split(':');
        if (parts.length !== 2) {
            console.log('Invalid format after base64 decode');
            return null;
        }
...
async function testRefreshFlow() {
    ...
    const testUser = {
        ...
        full_name: `Refresh Test ${timestamp}`,
        password: '123456',
    ...
    if (!decryptedReg || !decryptedReg.success) {
        ...
        console.log('Raw response:', registerRes.data);
        return;
    }
    
    console.log('✅ Registration successful!');
    console.log(`   User ID: ${decryptedReg.data.user.id}`);
