...
const { verifyToken, isMemberOfChama, isChairperson } = require('../../middleware/auth');

// Create meeting minutes (secretary or chairperson)
router.post('/chamas/:chamaId/meetings', verifyToken, async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Meeting minutes created', data: result.rows[0] });
    } catch (error) {
...
// Get all meetings
router.get('/chamas/:chamaId/meetings', verifyToken, isMemberOfChama, async (req, res) => {
    try {
        const { chamaId } = req.params;
...
// Get single meeting
router.get('/chamas/:chamaId/meetings/:meetingId', verifyToken, isMemberOfChama, async (req, res) => {
    try {
...
// Record attendance
router.post('/chamas/:chamaId/meetings/:meetingId/attendance', verifyToken, async (req, res) => {
    try {
        const { chamaId, meetingId } = req.params;
