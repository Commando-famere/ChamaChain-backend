...
const createMeeting = async (req, res) => {
    try {
        ...
        // Check if user is secretary, assistant secretary, or chairperson
        ...
        if (roleCheck.rows.length === 0) {
            return res.status(403).json({ success: false, message: 'Only secretary, assistant secretary, or chairperson can create meetings' });
        }
...
const updateMeeting = async (req, res) => {
    try {
        ...
        if (roleCheck.rows.length === 0) {
            return res.status(403).json({ success: false, message: 'Only secretary, assistant secretary, or chairperson can update meetings' });
        }
        ...
        await recordActivity(chamaId, 'meeting_updated', userId);
...
const deleteMeeting = async (req, res) => {
    try {
        ...
        if (roleCheck.rows.length === 0) {
            return res.status(403).json({ success: false, message: 'Only secretary or chairperson can delete meetings' });
        }

        await query(`DELETE FROM meeting_minutes WHERE id = $1 AND chama_id = $2`, [meetingId, chamaId]);
        await recordActivity(chamaId, 'meeting_deleted', userId);
...
const recordAttendance = async (req, res) => {
    try {
        ...
        if (roleCheck.rows.length === 0) {
            return res.status(403).json({ success: false, message: 'Only secretary, assistant secretary, or chairperson can record attendance' });
        }
...
// Get meeting history with attendance
...
module.exports = { ...module.exports, getMeetingHistory };
